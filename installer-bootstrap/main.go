...
func main(){
 ...
 // Start at every Windows logon in the interactive user session; agent itself runs without a console window.
 regCmd:=fmt.Sprintf(`reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "StaffMonitor" /t REG_SZ /d "\"%s\"" /f`,agentPath)
 _=runHidden("cmd.exe","/C",regCmd)
 c:=exec.Command(agentPath);c.SysProcAttr=&syscall.SysProcAttr{HideWindow:true};_ = c.Start()
