...
$('#deleteDeviceBtn').onclick = async () => {
  ...
  if (!confirm(`Kelola ${name}? Kamu bisa HAPUS PERMANEN agar PC boleh daftar ulang, atau NONAKTIFKAN agar PC tetap diblokir.`)) return;
  const purge = confirm('OK = HAPUS PERMANEN + history dan IZINKAN PC INI ENROLL ULANG.\nCancel = NONAKTIFKAN/REVOKE dan tetap BLOKIR PC.');
  ...
  await loadAll(true);
  notify(purge ? 'PC dihapus permanen. UID sudah dibebaskan dan boleh enroll ulang.' : 'PC dinonaktifkan/revoke dan tetap diblokir.');
};
