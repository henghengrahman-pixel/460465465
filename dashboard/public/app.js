...
function isNewDevice(d) {
  const stamp = d?.last_enrolled_at || d?.enrolled_at;
  if (!stamp) return false;
  const enrolled = new Date(stamp).getTime();
...
function renderDevices() {
  ...
  const list = devices
    .filter((d) => (!office || d.office_id === office) && deviceMatchesFilter(d, filter) &&
      [d.name, d.hostname, d.device_uid, d.morning_staff_name, d.night_staff_name, d.current_app, d.office_name]
