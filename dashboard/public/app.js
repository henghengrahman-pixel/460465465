...
function hydrateDashboardCache() {
  try {
    ...
    $('#subtitle').textContent = 'Realtime fleet monitoring · sinkronisasi…';
    return true;
  } catch { return false; }
}
...
async function boot() {
  ...
  try {
    ...
    setStats(r.stats || {});
    renderDevices();
    $('#subtitle').textContent = 'Realtime fleet monitoring';
    saveDashboardCache();
  } catch (err) {
    if (hadCache && !String(err.message || '').includes('Sesi login habis')) {
      $('#subtitle').textContent = 'Realtime fleet monitoring · koneksi ulang…';
...
async function command(type, payload) {
  ...
  alert('Perintah berhasil masuk antrean agent.');
}
$('#warnBtn').onclick = () => {
...
$('#deleteDeviceBtn').onclick = async () => {
  ...
  await loadAll(true);
  alert('Device sudah dihapus/revoke.');
};

async function loadOfficeTable() {
  await loadOffices();
  $('#officeRows').innerHTML = offices.map((o) => `<tr><td><b>${esc(o.name)}</b></td><td><span class="badge ${o.active ? 'ACTIVE' : 'OFFLINE'}">${o.active ? 'AKTIF' : 'NONAKTIF'}</span></td><td>${fmtDate(o.created_at)}</td><td>${user?.role === 'SUPER_ADMIN' ? `<button class="link office-edit" data-id="${o.id}">Edit</button> · <button class="link office-toggle" data-id="${o.id}">${o.active ? 'Nonaktifkan' : 'Aktifkan'}</button>` : '-'}</td></tr>`).join('');
}
...
function policyScopeOptions() {
  ...
  const rows = (offices || []).filter((o) => o.active);
  const allowAll = user?.role === 'SUPER_ADMIN';
  el.innerHTML = `${allowAll ? '<option value="ALL">ALL OFFICE</option>' : ''}${rows.filter((o) => allowAll || o.id === user?.officeId).map((o) => `<option value="${o.id}">${esc(o.name)}</option>`).join('')}`;
...
async function saveWebPolicy(enabled) {
  ...
  try {
    ...
    await loadWebPolicy();
    alert(enabled ? `Policy aktif. Dikirim ke ${r.fanout?.queued || 0} PC; ${r.fanout?.exempt || 0} PC dikecualikan.` : `Blokir dibuka. Perintah unlock dikirim ke ${r.fanout?.queued || 0} PC.`);
  } finally { setBusy(btn, false); }
}
