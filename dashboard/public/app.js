...
function hideViews() {
  ['dashboardView', 'deviceDetail', 'officesView', 'staffView', 'usersView', 'auditView']
...
async function boot() {
  ...
  try {
    ...
    await Promise.all([loadOffices(), loadStaffData()]);
    await loadAll(true);
  } catch (err) {
    localStorage.removeItem('sm_token');
    token = null;
...
async function loadStats() {
  ...
  $('#online').textContent = r.online;
  $('#active').textContent = r.active;
  $('#idle').textContent = r.idle;
}
...
async function loadAll(force = false) {
  ...
  allLoadPromise = Promise.all([loadStats(), loadDevices()])
...
async function openDevice(id) {
  ...
  const [r, a] = await Promise.all([api(`/api/devices/${id}`), api(`/api/devices/${id}/activity`)]);
  if (seq !== detailLoadSeq || currentView !== 'device') return;
  if (selected?.id && selected.id !== id) socket.emit('unwatchDevice', selected.id);
  selected = r.item;
  renderDeviceInfo(selected);
