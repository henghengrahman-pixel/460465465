...
function hideViews() {
  ['dashboardView', 'deviceDetail', 'officesView', 'staffView', 'policyView', 'usersView', 'auditView', 'revokedView']
...
function isNewDevice(d) {
  const raw = d?.last_enrolled_at || d?.enrolled_at;
  if (!raw) return false;
  const enrolled = new Date(raw).getTime();
  const ageMs = Date.now() - enrolled;
  return Number.isFinite(enrolled) && ageMs >= 0 && ageMs <= 24 * 60 * 60 * 1000;
}

function deviceMatchesFilter(d, filter) {
  if (!filter) return true;
  if (filter === 'NEW_24H') return isNewDevice(d);
  if (filter === 'UNASSIGNED_OFFICE') return !d.office_id;
  if (filter === 'UNASSIGNED_STAFF') return !d.morning_staff_id && !d.night_staff_id;
  if (filter === 'ONLINE') return ['ONLINE', 'ACTIVE', 'IDLE', 'LOCKED', 'SYSTEM_ONLY'].includes(String(d.effective_status || d.status || '').toUpperCase());
  if (filter === 'OFFLINE') return String(d.effective_status || d.status || '').toUpperCase() === 'OFFLINE';
  return true;
}
...
function renderDevices() {
  ...
  const list = devices
    .filter((d) => (!office || d.office_id === office) && deviceMatchesFilter(d, filter) &&
      [d.name, d.hostname, d.device_uid, d.morning_staff_name, d.morning_staff_code,
        d.night_staff_name, d.night_staff_code, d.current_app, d.current_title, d.office_name]
        .some((v) => String(v || '').toLowerCase().includes(q)))
    .sort((a, b) => filter === 'NEW_24H'
      ? new Date(b.last_enrolled_at || b.enrolled_at || 0) - new Date(a.last_enrolled_at || a.enrolled_at || 0)
      : String(a.name || '').localeCompare(String(b.name || '')));
  $('#deviceRows').innerHTML = list.map((d) => `<tr>
    <td><button class="link device-open" data-id="${d.id}">${esc(d.name)}</button>${isNewDevice(d) ? ' <span class="badge NEW">BARU</span>' : ''}<div class="muted mono">${esc((d.device_uid || '').slice(0, 14))}…</div></td>
  ...
  </tr>`).join('') || '<tr><td colspan="8">Belum ada device sesuai filter.</td></tr>';
}
...
$('#deviceFilter').onchange = renderDevices;
...
function renderDeviceInfo(d) {
  ...
  $('#deviceInfo').innerHTML = `<div class="info-grid">
    ...
    <div><span>Hostname Windows</span><b>${esc(d.hostname || d.name || '-')}</b></div>
...
$('#deleteDeviceBtn').onclick = async () => {
  ...
  const uid = selected.device_uid;
  if (!confirm(`Hapus ${name} dari dashboard?\n\nPC akan langsung diblokir agar agent yang masih hidup TIDAK muncul kembali. Kamu dapat mengizinkannya lagi dari menu PC Diblokir.`)) return;
  ...
  await loadAll(true);
  notify(`PC ${name} berhasil dihapus dan diblokir. UID ${uid} tidak dapat enroll ulang sampai diizinkan lagi.`);
};
...
async function loadUsers() {
  try {
    ...
    $('#userRows').innerHTML = r.items.map((x) => {
      const isMaster = x.login_id === masterId;
      const actions = isMaster
        ? '<span class="muted">Master (ENV)</span>'
        : `<button class="link user-reset" data-id="${x.id}">Reset Password</button> · <button class="link user-delete danger-link" data-id="${x.id}">Hapus</button>`;
      return `<tr><td>${esc(x.login_id)}${isMaster ? '<div class="muted">MASTER</div>' : ''}</td><td>${esc(x.display_name)}</td><td>${esc(x.role)}</td><td>${esc(x.office_name || 'ALL OFFICE')}</td><td>${x.active ? 'AKTIF' : 'NONAKTIF'}</td><td>${fmtDate(x.last_login_at)}</td><td>${actions}</td></tr>`;
    }).join('');
  } catch (e) { $('#userRows').innerHTML = `<tr><td colspan="7">${esc(e.message)}</td></tr>`; }
}
$('#userRows').addEventListener('click',(e)=>{
  const btn=e.target.closest('button[data-id]'); if(!btn)return;
  if(btn.classList.contains('user-reset')) resetAdminPassword(btn.dataset.id).catch((err)=>notify(err.message||'Gagal reset password.','error'));
  if(btn.classList.contains('user-delete')) deleteAdminUser(btn.dataset.id).catch((err)=>notify(err.message||'Gagal menghapus user.','error'));
});
async function resetAdminPassword(id){
  if(user?.role!=='SUPER_ADMIN')return;
  const row=[...document.querySelectorAll('#userRows tr')].find((tr)=>tr.querySelector(`[data-id="${id}"]`));
  const loginId=row?.children?.[0]?.textContent?.replace('MASTER','').trim()||'user';
  showModal(`Reset Password · ${loginId}`,`<label>Password Baru<input name="password" type="password" minlength="10" maxlength="200" autocomplete="new-password" placeholder="Minimal 10 karakter" required></label><label>Ulangi Password<input name="confirm" type="password" minlength="10" maxlength="200" autocomplete="new-password" placeholder="Ulangi password baru" required></label><div class="hint">Password akun master utama tetap dikelola dari Railway ENV ADMIN_PASSWORD.</div>`,async(fd)=>{
    const password=String(fd.get('password')||''); const confirmPw=String(fd.get('confirm')||'');
    if(password!==confirmPw)throw new Error('Konfirmasi password tidak sama.');
    await api(`/api/users/${id}/reset-password`,{method:'POST',body:JSON.stringify({password})});
    await loadUsers(); notify('Password admin berhasil direset.');
  });
}
async function deleteAdminUser(id){
  ...
  const row=[...document.querySelectorAll('#userRows tr')].find((tr)=>tr.querySelector(`[data-id="${id}"]`));
  const loginId=row?.children?.[0]?.textContent?.replace('MASTER','').trim()||'user';
  if(!confirm(`Hapus akun admin ${loginId}? Akun ini langsung tidak bisa login lagi.`))return;
  const typed=prompt(`Ketik HAPUS ${loginId} untuk konfirmasi:`,'');
  if(typed!==`HAPUS ${loginId}`)return notify('Penghapusan dibatalkan.','error');
  await api(`/api/users/${id}`,{method:'DELETE'});
  await loadUsers(); notify(`Akun ${loginId} berhasil dihapus.`);
}
$('#addUser').onclick = () => {
...
async function loadRevoked() {
  if (user?.role !== 'SUPER_ADMIN') return;
  const r = await api('/api/revoked-devices', { timeoutMs: 8000 });
  const rows = r.items || [];
  $('#revokedRows').innerHTML = rows.map((x) => `<tr><td class="mono">${esc(x.device_uid)}</td><td>${esc(x.reason || 'Diblokir')}</td><td>${fmtDate(x.revoked_at)}</td><td><button class="link revoked-reallow" data-uid="${esc(x.device_uid)}">IZINKAN LAGI</button></td></tr>`).join('') || '<tr><td colspan="4">Tidak ada PC yang diblokir.</td></tr>';
}
$('#reloadRevoked').onclick = () => loadRevoked().catch((e) => notify(e.message || 'Gagal memuat PC diblokir.', 'error'));
$('#revokedRows').addEventListener('click', (e) => {
  const btn = e.target.closest('.revoked-reallow'); if (!btn) return;
  const uid = btn.dataset.uid;
  if (!confirm(`Izinkan Device UID ${uid} enroll kembali?\n\nJika agent di PC masih berjalan, PC akan muncul lagi otomatis sebagai device baru.`)) return;
  api('/api/revoked-devices/reallow', { method: 'POST', body: JSON.stringify({ deviceUid: uid }) })
    .then(async () => { await loadRevoked(); await loadAll(true); notify('PC sudah diizinkan enroll kembali. Agent akan masuk otomatis saat retry.'); })
    .catch((err) => notify(err.message || 'Gagal membuka blokir PC.', 'error'));
});
...
async function navigate(view) {
  ...
  if (view === 'revoked') { if (user?.role !== 'SUPER_ADMIN') return navigate('dashboard'); $('#revokedView').classList.remove('hidden'); $('#title').textContent = 'PC Diblokir'; await loadRevoked(); }
}
