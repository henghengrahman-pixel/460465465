...
app.post('/api/agent/enroll',asyncH(async(req,res)=>{
 ...
 if(!existing.rows[0]&&hardwareUid){
   const byHardware=await q('SELECT id,device_uid,name,office_id,hardware_uid FROM devices WHERE hardware_uid=$1 AND disabled=false ORDER BY COALESCE(last_enrolled_at,enrolled_at) DESC LIMIT 2',[hardwareUid]);
   if(byHardware.rows.length===1)existing=byHardware;
 }
 const created=!existing.rows[0];
 let d;
 if(existing.rows[0]){
   if(existing.rows[0].device_uid!==s.deviceUid){
     const collision=await q('SELECT id FROM devices WHERE device_uid=$1 AND id<>$2',[s.deviceUid,existing.rows[0].id]);
     if(!collision.rows[0])await q('UPDATE devices SET device_uid=$1 WHERE id=$2',[s.deviceUid,existing.rows[0].id]);
   }
   d=await q(`UPDATE devices SET hostname=$2,os=$3,agent_version=$4,office_id=COALESCE(office_id,$5),
         hardware_uid=COALESCE(hardware_uid,$6),system_last_seen=now(),
         last_enrolled_at=now(),disabled=false
     WHERE id=$1 RETURNING *`,
     [existing.rows[0].id,s.name,s.os||null,s.agentVersion||null,officeId,hardwareUid]);
 }else{
   d=await q(`INSERT INTO devices(id,device_uid,hardware_uid,name,hostname,os,agent_version,office_id,status,system_last_seen,last_enrolled_at) VALUES(gen_random_uuid(),$1,$2,$3,$3,$4,$5,$6,'ONLINE',now(),now()) RETURNING *`,
     [s.deviceUid,hardwareUid,s.name,s.os||null,s.agentVersion||null,officeId]);
 }
 ...
 console.log('[ENROLL]',JSON.stringify({deviceId:d.rows[0].id,deviceUid:d.rows[0].device_uid,hostname:d.rows[0].hostname,name:d.rows[0].name,created,officeAssigned,officeId:d.rows[0].office_id||null,requestedOffice:requestedOffice||null,resolvedOfficeName}));
 queueFleetChange(d.rows[0].id);
 res.set('Cache-Control','no-store').json({ok:true,created,deviceId:d.rows[0].id,deviceUid:d.rows[0].device_uid,deviceToken:token,officeAssigned,officeId:d.rows[0].office_id||null,officeName:resolvedOfficeName,lastEnrolledAt:d.rows[0].last_enrolled_at,warning:officeAssigned?null:'PC terdaftar tetapi belum mendapat kantor. Tampil di filter BELUM ADA KANTOR / PC BARU.'});
}));
