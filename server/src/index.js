...
app.post('/api/agent/enroll',asyncH(async(req,res)=>{
 ...
 // Kompatibilitas agent lama: beberapa build mengirim hardwareUid kosong / pendek.
 // hardwareUid hanya dipakai sebagai secondary identity, jadi jangan gagalkan enrollment karena field opsional ini.
 const s=z.object({deviceUid:z.string().trim().min(8).max(200),hardwareUid:z.union([z.string().max(200),z.null()]).optional(),name:z.string().trim().min(1).max(200),os:z.string().max(200).optional(),agentVersion:z.string().max(50).optional(),officeName:z.string().max(200).optional()}).parse(req.body||{});
 ...
 if(!existing.rows[0]&&hardwareUid)existing=await q('SELECT id,device_uid,name,office_id,hardware_uid FROM devices WHERE hardware_uid=$1 AND disabled=false ORDER BY enrolled_at LIMIT 1',[hardwareUid]);
 const created=!existing.rows[0];
 let d;if(existing.rows[0]){d=await q(`UPDATE devices SET name=$2,os=$3,agent_version=$4,office_id=COALESCE(office_id,$5),hardware_uid=COALESCE(hardware_uid,$6),system_last_seen=now(),disabled=false WHERE id=$1 RETURNING *`,[existing.rows[0].id,s.name,s.os||null,s.agentVersion||null,officeId,hardwareUid]);}
 else{d=await q(`INSERT INTO devices(id,device_uid,hardware_uid,name,os,agent_version,office_id,status,system_last_seen) VALUES(gen_random_uuid(),$1,$2,$3,$4,$5,$6,'ONLINE',now()) RETURNING *`,[s.deviceUid,hardwareUid,s.name,s.os||null,s.agentVersion||null,officeId]);}
 const token=makeToken();await q('UPDATE device_tokens SET revoked_at=now() WHERE device_id=$1 AND revoked_at IS NULL',[d.rows[0].id]);await q('INSERT INTO device_tokens(id,device_id,token_hash) VALUES(gen_random_uuid(),$1,$2)',[d.rows[0].id,tokenHash(token)]);
 const officeAssigned=!!d.rows[0].office_id;
 console.log('[ENROLL]',JSON.stringify({deviceId:d.rows[0].id,deviceUid:d.rows[0].device_uid,name:d.rows[0].name,created,officeAssigned,requestedOffice:requestedOffice||null,resolvedOfficeName}));
 queueFleetChange(d.rows[0].id);
 res.json({ok:true,created,deviceId:d.rows[0].id,deviceUid:d.rows[0].device_uid,deviceToken:token,officeAssigned,officeId:d.rows[0].office_id||null,officeName:resolvedOfficeName,warning:officeAssigned?null:'PC terdaftar tetapi belum mendapat kantor. Tampil di filter BELUM ADA KANTOR / PC BARU.'});
}));
...
app.delete('/api/devices/:id',requireAuth,requireRole('SUPER_ADMIN'),asyncH(async(req,res)=>{const mode=String(req.query.mode||'revoke');const d=await q('SELECT * FROM devices WHERE id=$1',[req.params.id]);if(!d.rows[0])return res.status(404).json({ok:false,error:'Device not found'});const uid=d.rows[0].device_uid;if(mode==='purge'){// Hapus permanen = lupakan device sepenuhnya dan IZINKAN PC yang sama enroll kembali.
  await q('DELETE FROM revoked_devices WHERE device_uid=$1',[uid]);
  await q('DELETE FROM devices WHERE id=$1',[req.params.id]);
}else{// Revoke/nonaktifkan = device tetap diblokir sampai SUPER_ADMIN mengizinkan kembali.
  await q('INSERT INTO revoked_devices(device_uid,reason) VALUES($1,$2) ON CONFLICT(device_uid) DO UPDATE SET revoked_at=now(),reason=excluded.reason',[uid,'Revoked from dashboard']);
  await q('UPDATE device_tokens SET revoked_at=now() WHERE device_id=$1 AND revoked_at IS NULL',[req.params.id]);
  await q("UPDATE devices SET disabled=true,status='OFFLINE' WHERE id=$1",[req.params.id]);
}liveFrames.delete(req.params.id);await audit(req,mode==='purge'?'DELETE_DEVICE_PURGE':'REVOKE_DEVICE',d.rows[0].name,{deviceUid:uid,reEnrollAllowed:mode==='purge'});io.emit('fleet.changed',{});res.json({ok:true,reEnrollAllowed:mode==='purge'});}));
