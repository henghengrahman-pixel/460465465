...
app.post('/api/agent/enroll',asyncH(async(req,res)=>{
 ...
 let d;if(existing.rows[0]){d=await q(`UPDATE devices SET hostname=$2,os=$3,agent_version=$4,office_id=COALESCE(office_id,$5),hardware_uid=COALESCE(hardware_uid,$6),system_last_seen=now(),last_enrolled_at=now(),disabled=false WHERE id=$1 RETURNING *`,[existing.rows[0].id,s.name,s.os||null,s.agentVersion||null,officeId,hardwareUid]);}
 else{d=await q(`INSERT INTO devices(id,device_uid,hardware_uid,name,hostname,os,agent_version,office_id,status,system_last_seen,last_enrolled_at) VALUES(gen_random_uuid(),$1,$2,$3,$3,$4,$5,$6,'ONLINE',now(),now()) RETURNING *`,[s.deviceUid,hardwareUid,s.name,s.os||null,s.agentVersion||null,officeId]);}
...
app.get('/api/devices',requireAuth,asyncH(async(req,res)=>{const o=officeSql(req.user,'d');const r=await q(`${deviceSelect} WHERE d.disabled=false ${o.clause} ORDER BY d.name LIMIT 1000`,o.params);res.set('Cache-Control','no-store');res.json({ok:true,items:r.rows});}));
