...
app.post('/api/agent/enroll',asyncH(async(req,res)=>{
 ...
 let d;if(existing.rows[0]){d=await q(`UPDATE devices SET hostname=$2,os=$3,agent_version=$4,office_id=COALESCE(office_id,$5),hardware_uid=COALESCE(hardware_uid,$6),system_last_seen=now(),disabled=false WHERE id=$1 RETURNING *`,[existing.rows[0].id,s.name,s.os||null,s.agentVersion||null,officeId,hardwareUid]);}
 else{d=await q(`INSERT INTO devices(id,device_uid,hardware_uid,name,hostname,os,agent_version,office_id,status,system_last_seen) VALUES(gen_random_uuid(),$1,$2,$3,$3,$4,$5,$6,'ONLINE',now()) RETURNING *`,[s.deviceUid,hardwareUid,s.name,s.os||null,s.agentVersion||null,officeId]);}
