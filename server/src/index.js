...
function resolvePolicyScope(user,requested){
  if(user.role==='SUPER_ADMIN'){
    if(!requested||requested==='ALL')return {scopeKey:'ALL',officeId:null};
    return {scopeKey:String(requested),officeId:String(requested)};
  }
  if(!user.officeId)throw Object.assign(new Error('Admin tidak memiliki kantor'),{statusCode:400});
  return {scopeKey:String(user.officeId),officeId:String(user.officeId)};
}
async function policySnapshot(user,requestedScope){
  const scope=resolvePolicyScope(user,requestedScope);
  if(scope.officeId){const o=await q('SELECT id,name FROM offices WHERE id=$1',[scope.officeId]);if(!o.rows[0])throw Object.assign(new Error('Kantor tidak ditemukan'),{statusCode:404});}
...
async function policyTargets(scopeKey,officeId,exceptionStaffIds=[]){
  const params=[];let where="d.disabled=false";
  if(officeId){params.push(officeId);where+=` AND d.office_id=$${params.length}`;}
  const r=await q(`SELECT d.id,d.name,d.office_id,
    EXISTS(SELECT 1 FROM device_shift_assignments x WHERE x.device_id=d.id AND x.staff_id = ANY($${params.length+1}::uuid[])) exempt
    FROM devices d WHERE ${where} ORDER BY d.name`,[...params,exceptionStaffIds]);
  return r.rows;
}
async function fanoutWebPolicy({scopeKey,officeId,enabled,domains,exceptionStaffIds,version,userId}){
  const targets=await policyTargets(scopeKey,officeId,exceptionStaffIds);
  ...
  try{
    ...
    for(const d of targets){
      ...
      await client.query("INSERT INTO commands(device_id,command_type,payload,created_by) VALUES($1,'SET_POLICY',$2,$3)",[d.id,payload,userId]);
      queued++;
    }
...
app.post('/api/agent/heartbeat',deviceAuth,asyncH(async(req,res)=>{
 ...
 const pol=await q(`SELECT p.scope_key,p.enabled,p.blocked_domains,p.version,
   ...
   FROM web_policies p WHERE p.scope_key IN ('ALL',COALESCE($2::text,'')) ORDER BY CASE WHEN p.scope_key=$2::text THEN 0 ELSE 1 END LIMIT 1`,[req.device.id,req.device.office_id]);
...
async function getFleet(user){const o=officeSql(user,'d');const r=await q(`${deviceSelect} WHERE d.disabled=false ${o.clause} ORDER BY d.name LIMIT 1000`,o.params);return r.rows;}
...
app.get('/api/web-policy',requireAuth,requireRole('SUPER_ADMIN','ADMIN','SUPERVISOR'),asyncH(async(req,res)=>{
 ...
 if(snap.scope.officeId){staffParams.push(snap.scope.officeId);sw+=` AND s.office_id=$${staffParams.length}`;}
 ...
 const targets=await policyTargets(snap.scope.scopeKey,snap.scope.officeId,exceptionIds);
...
app.put('/api/web-policy',requireAuth,requireRole('SUPER_ADMIN','ADMIN'),asyncH(async(req,res)=>{
 ...
 const scope=resolvePolicyScope(req.user,b.scope);const domains=normalizeDomains(b.domains);
 if(b.enabled&&!domains.length)return res.status(400).json({ok:false,error:'Policy aktif membutuhkan minimal 1 domain valid'});
 if(scope.officeId){const o=await q('SELECT id FROM offices WHERE id=$1 AND active=true',[scope.officeId]);if(!o.rows[0])return res.status(400).json({ok:false,error:'Kantor tidak aktif/tidak ditemukan'});}
 ...
 const fanout=await fanoutWebPolicy({scopeKey:scope.scopeKey,officeId:scope.officeId,enabled:b.enabled,domains,exceptionStaffIds:b.exceptionStaffIds,version,userId:req.user.sub});
...
app.post('/api/web-policy/reapply',requireAuth,requireRole('SUPER_ADMIN','ADMIN'),asyncH(async(req,res)=>{
 ...
 const fanout=await fanoutWebPolicy({scopeKey:snap.scope.scopeKey,officeId:snap.scope.officeId,enabled:!!snap.policy.enabled,domains,exceptionStaffIds:ids,version:Number(snap.policy.version||0),userId:req.user.sub});
...
app.use((err,req,res,next)=>{console.error(err);if(err?.name==='ZodError')return res.status(400).json({ok:false,error:'Validation error',detail:err.issues});if(err?.code==='23505')return res.status(409).json({ok:false,error:'Data sudah digunakan / duplikat'});res.status(err?.statusCode||500).json({ok:false,error:err?.statusCode?err.message:'Internal server error'});});
