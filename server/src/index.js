...
app.post('/api/auth/login',asyncH(async(req,res)=>{
 ...
 if(!cfg.configured&&cfg.source==='data-dir'){payload.setupSecret=cfg.secret;payload.otpauthUri=makeOtpAuthUri({secret:cfg.secret,account:user.loginId,issuer:process.env.ADMIN_2FA_ISSUER||'Staff Management'});}
 res.json(payload);
}));
