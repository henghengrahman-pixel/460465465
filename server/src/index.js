...
app.post('/api/agent/enroll',asyncH(async(req,res)=>{
 ...
 else{d=await q(`INSERT INTO devices(id,device_uid,hardware_uid,name,os,agent_version,office_id,status,system_last_seen) VALUES(gen_random_uuid(),$1,$2,$3,$4,$5,$6,'ONLINE',now()) RETURNING *`,[s.deviceUid,s.hardwareUid||null,s.name,s.os||null,s.agentVersion||null,officeId]);}
 const token=makeToken();await q('UPDATE device_tokens SET revoked_at=now() WHERE device_id=$1 AND revoked_at IS NULL',[d.rows[0].id]);await q('INSERT INTO device_tokens(id,device_id,token_hash) VALUES(gen_random_uuid(),$1,$2)',[d.rows[0].id,tokenHash(token)]);res.json({ok:true,deviceId:d.rows[0].id,deviceUid:d.rows[0].device_uid,deviceToken:token});
}));
...
app.put('/api/devices/:id/config',requireAuth,requireRole('SUPER_ADMIN','ADMIN'),asyncH(async(req,res)=>{
 ...
 try{
  ...
  for(const [shift,id] of [['PAGI',s.morningStaffId],['MALAM',s.nightStaffId]]){
   if(id)await client.query(`INSERT INTO device_shift_assignments(id,device_id,shift,staff_id,updated_at) VALUES(gen_random_uuid(),$1,$2,$3,now()) ON CONFLICT(device_id,shift) DO UPDATE SET staff_id=excluded.staff_id,updated_at=now()`,[req.params.id,shift,id]);
   else await client.query('DELETE FROM device_shift_assignments WHERE device_id=$1 AND shift=$2',[req.params.id,shift]);
  }
  await client.query('COMMIT');
...
function friendlyDbError(err){
 ...
 if(code==='22P02')return {status:400,message:'Format data tidak valid. Refresh dashboard lalu coba simpan kembali.'};
