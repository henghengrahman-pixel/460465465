...
const hasAgent=fs.existsSync('agent/main.go');
const agent=hasAgent?fs.readFileSync('agent/main.go','utf8'):'';
const server=fs.readFileSync('server/src/index.js','utf8');
const ui=fs.readFileSync('dashboard/public/app.js','utf8');
const hasInstaller=fs.existsSync('install/Install-StaffMonitor.ps1');
const installer=hasInstaller?fs.readFileSync('install/Install-StaffMonitor.ps1','utf8'):'';

test('v2 agent uses watchdog and never stops permanently on enrollment outage',{skip:!hasAgent},()=>{
...
test('v2 installation uses ProgramData no-space binary path and clean upgrade',{skip:!hasInstaller},()=>{
